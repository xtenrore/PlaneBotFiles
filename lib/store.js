...
const { applyOfficialRosterPatch } = require('./official-roster-patch');

function canonicalSeed() {
  return applyOfficialRosterPatch(seedData());
}

class Store {
  constructor(filePath) {
    this.filePath = filePath;
    this.data = canonicalSeed();
    this.load();
  }

  load() {
    try {
      const fresh = canonicalSeed();
      if (fs.existsSync(this.filePath)) {
        ...
        // current fully audited 2026/27 TFF A-team data pack.
        ...
        const hasCanonicalRealPlayers = parsedPlayers.some(p => p && p.name === 'Victor Osimhen' && p.club === 'Galatasaray' && p.realPlayer === true)
          ...
          && parsedPlayers.some(p => p && p.name === 'Kouadou Jaures Assoumou' && p.club === 'Samsunspor')
          && parsedPlayers.some(p => p && p.name === 'Enis Destan' && p.club === 'Tümosan Konyaspor')
          && parsedPlayers.some(p => p && p.name === 'Gaius Makouta' && p.club === 'Corendon Alanyaspor')
          && parsedPlayers.some(p => p && p.name === 'Charles Raux-Yao' && p.club === 'Eyüpspor')
          && parsedPlayers.some(p => p && p.name === 'Tobias Gulliksen' && p.club === 'Kocaelispor');
        ...
        if (needsCanonicalRefresh) {
          this.data = {
            ...
            ...parsed,
            version: newVersion,
            footballData: fresh.footballData,
          ...
          this.save();
          console.log(`Canonical TFF 2026/27 futbol verisi yenilendi (v${oldVersion} -> v${newVersion}); kullanıcı ilerlemesi korundu.`);
        }
      } else {
        this.data = fresh;
        this.save();
