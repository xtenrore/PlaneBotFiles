...
function sanitizeSeededFixtures(data) {
  if (!Array.isArray(data.fixtures)) return;
  for (const fixture of data.fixtures) {
    const noScore = fixture.homeScore === null || fixture.homeScore === undefined || fixture.awayScore === null || fixture.awayScore === undefined;
    // A seed/fallback fixture may provide a kickoff but must never invent that a
    // match has finished. Only a verified live provider/admin update can set a
    // scoreless match to a terminal state.
    if (noScore && ['BİTTİ', 'CANLI'].includes(String(fixture.status || '').toUpperCase())) {
      fixture.status = 'VERİ BEKLENİYOR';
      fixture.minute = null;
      fixture.dataSource = 'fallback';
      fixture.verifiedResult = false;
    }
  }
}
...
function applyOfficialRosterPatch(data) {
  ...
  sanitizeSeededFixtures(data);

  // Version 5 removes fabricated match state from fallback fixtures. A match is
  // only live/finished once a verified match-data provider (or admin sync) says so.
  ...
  data.footballData = {
    ...
    fixturePolicy: 'verified-provider-only-for-live-and-final-state'
  };
  return data;
}
